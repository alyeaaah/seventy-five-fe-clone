...
const nestedMenu = (menu: Array<Menu | "divider">, location: Location) => {
  ...
  menu.forEach((item) => {
    if (typeof item !== "string") {
      ...
      const menuRoute = menuItem.pathname?.replace("/admin", "").split("/");
      const currentRoute = location.pathname?.replace("/admin", "").split("/");
      const mainRoute = "/" + (menuRoute?.length && menuRoute?.length > 1? menuRoute[1] : "");
      const routeSubMenu = "/" + (currentRoute?.length && currentRoute.length > 2 ? currentRoute[2] : "");

      const shouldActive = location.pathname.includes(menuItem.pathname || "");
      ...
      menuItem.active =
        ((location.forceActiveMenu !== undefined &&
          ...
          (location.forceActiveMenu === undefined &&
            menuItem.pathname === location.pathname || (mainRoute !== "/" && location.pathname.includes(mainRoute) && shouldActive)) ||
