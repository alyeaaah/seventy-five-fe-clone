    ...
    <div className={`bracket-seed-container `} key={`seed-container-${roundIndex}-${seedIndex}`}>
      <div className={`bracket-seed bg-white dark:bg-darkmode-600 ${readOnly ? 'hover:transform hover:scale-105 duration-700 transition-all hover:z-10' : ''}`} onClick={() => !!onSeedClick ? onSeedClick(seed, seedIndex, roundIndex) : null}>
        ...
        <div ref={dropHomeRef} className='bracket-team' style={{ borderRadius: 12, backgroundColor: isOverHome ? '#d0cebf' : "unset" }} key={`team-home-${roundIndex}-${seedIndex}`}>
          {isOverHome ? <div className='h-[45px] flex justify-center items-center text-xs font-medium' key={`drop-home-${roundIndex}-${seedIndex}`}>Drop Team Here</div> : <DraggableTeam team={seed.teams[0]} readOnly={readOnly} draggable={!roundIndex} key={`team-home-${roundIndex}-${seedIndex}-${seed.teams[0]?.id || 'home'}`} />}
        </div>
        <div ref={dropAwayRef} className='bracket-team' style={{ borderRadius: 12, backgroundColor: isOverAway ? '#d0cebf' : "unset" }} key={`team-away-${roundIndex}-${seedIndex}`}>
          {isOverAway ? <div className='h-[45px] flex justify-center items-center text-xs font-medium' key={`drop-away-${roundIndex}-${seedIndex}`}>Drop Team Here</div> : <DraggableTeam team={seed.teams[1]} readOnly={readOnly} draggable={!roundIndex} key={`team-away-${roundIndex}-${seedIndex}-${seed.teams[1]?.id || 'away'}`} isAway />}
        </div>
        {
          (seed.court || seed.time) &&
          <div className='absolute top-[96px] w-full z-10 flex justify-center cursor-pointer' key={`match-info-${roundIndex}-${seedIndex}`}>
            <div className='bg-emerald-800 text-white rounded-lg shadow-xl flex flex-col items-center justify-between pl-1 pr-2'>
              {seed?.court &&
                <div className='flex items-center' key={`court-info-${roundIndex}-${seedIndex}`}>
                  <Lucide icon="MapPin" className='h-3' />
                  <span className='text-[10px] font-medium line-clamp-1 text-ellipsis'>{seed?.court} </span>
                </div>
              }
              {seed?.time &&
                <div className='flex items-center' key={`time-info-${roundIndex}-${seedIndex}`}>
