...
function Main() {
  ...
  const mainMenuPath = subMenuOf && !subMenuOf.includes("master data") ? `/admin/${location.pathname.split("/")[2]}/` : paths.administrator.dashboard
    ...
    <>
      ...
      <div className="h-[67px] z-[51] flex items-center relative border-b border-slate-200">
        ...
        <Breadcrumb className="hidden mr-auto -intro-x sm:flex">
          <Breadcrumb.Link to={paths.administrator.dashboard}>75 Administrator</Breadcrumb.Link>
          {/* {
            subMenuOf ?
          ...
          {location.pathname.split("/").length > 2 ? (
            <Breadcrumb.Link key={4} to="#" active={true} className="capitalize">
              {location.pathname.split("/")[3]}
            </Breadcrumb.Link>
          ) : <></>
          }
          {location.pathname.split("/").length > 4 ? (
            <Breadcrumb.Link key={5} to="#" active={true} className="capitalize">
        ...
        <Menu>
          ...
          <Menu.Items className="w-56 mt-px text-white bg-primary">
            ...
            <Menu.Divider className="bg-white/[0.08]" />
            <Menu.Item className="hover:bg-white/5">
              <Link to={paths.player.home} replace={true} target="_blank" className="flex flex-row items-center">
                <Lucide icon="RefreshCw" className="w-4 h-4 mr-2" /> Login as Player
              </Link>
            </Menu.Item>
            <Menu.Item className="hover:bg-white/5"
              onClick={() => {
