    ...
    <div className={`box rounded-2xl flex flex-col items-center justify-stretch overflow-hidden relative ${className}`} {...props}>
      ...
      <div className="flex flex-col w-full overflow-scroll">
        {playerStanding?.data?.map((pl, index) => (
          <div
            key={index}
            className={`flex flex-row items-center justify-between w-full py-2 px-3 ${index % 2 === 1 ? 'bg-gray-100 dark:bg-darkmode-800' : ''} ${pl.uuid === player ? 'border border-emerald-800 dark:border-emerald-400 rounded' : ''}`}
            onClick={() => navigate(paths.players.info({ uuid: pl.uuid || "" }).$)}
          >
