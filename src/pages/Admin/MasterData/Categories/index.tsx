...
import { CategoriesData } from "./api/schema";
...
function Categories() {
  ...
  const [tableData, setTableData] = useState<CategoriesData[]>([]);
  const [isModalCreateOpen, setIsModalCreateOpen] = useState(false);
  const [modalAlert, setModalAlert] = useState<AlertProps | undefined>(undefined);
  const [selectedCategory, setSelectedCategory] = useState<Partial<CategoriesData> | undefined>();
  const { showNotification } = useToast();
  const { mutate: actionDeleteCategory } = CategoriesApiHooks.useDeleteCategory({
    params: {
  ...
  const { data, isLoading, refetch } = CategoriesApiHooks.useGetCategoriesList(
    {
      queries: {
        page,
        limit,
  ...
  const tableColumnsAntd: ColumnsType<CategoriesData> = [
    {
      title: "",
      dataIndex: "id",
      align: "left",
                  ...
                  buttons: [
                    ...
                    },
                    {
                      ...
                      onClick: () => {
                        ...
                        actionDeleteCategory(undefined)
                      },
                      variant: "danger"
                    }
                  ]
            ...
            <div className="items-center mt-2 sm:mr-4 xl:mt-0">
              <FormInput
                ...
                onChange={(e) => {
                  ...
                  if (!e.target.value) {
                    queryClient.invalidateQueries({ queryKey: CategoriesApiHooks.getKeyByAlias("getCategoriesList") });
                  }
                }}
                type="text"
