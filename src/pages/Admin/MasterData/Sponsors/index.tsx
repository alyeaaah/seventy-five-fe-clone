...
import { SponsorsData } from "./api/schema";
...
function Sponsors() {
  ...
  const [selectedSponsor, setSelectedSponsor] = useState<Partial<SponsorsData> | undefined>();
  const { showNotification } = useToast();
  const { mutate: actionDeleteSponsor } = SponsorsApiHooks.useDeleteSponsor({
    params: {
  ...
  const { data, isLoading, refetch } = SponsorsApiHooks.useGetSponsorsList(
    {
      queries: {
        page,
        limit,
  ...
  return (
    <>
      <div className="flex flex-row items-center mt-8 intro-y justify-between">
        <h2 className="mr-auto text-lg font-medium">Sponsors</h2>
        <div className="flex">
            ...
            <div className="items-center mt-2 sm:mr-4 xl:mt-0">
              <FormInput
                ...
                onChange={(e) => {
                  ...
                  if (!e.target.value) {
                    queryClient.invalidateQueries({ queryKey: SponsorsApiHooks.getKeyByAlias("getSponsorsList") });
                  }
                }}
                type="text"
