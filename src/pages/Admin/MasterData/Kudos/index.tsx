...
import { KudosData } from "./api/schema";
...
function Kudos() {
  ...
  const [selectedKudos, setSelectedKudos] = useState<Partial<KudosData> | undefined>();
  const { showNotification } = useToast();
  const { mutate: actionDeleteKudos } = KudosApiHooks.useDeleteKudos({
    params: {
    ...
    onSuccess: () => {
      showNotification({
        duration: 3000,
        text: "Kudos deleted successfully",
        icon: "WashingMachine",
        variant: "danger",
      });
  ...
  const { data, isLoading, refetch } = KudosApiHooks.useGetKudosList(
    {
      queries: {
        page,
        limit,
  ...
  const tableColumnsAntd: ColumnsType<KudosData> = [
    {
      title: "",
      dataIndex: "id",
      align: "left",
  ...
  return (
    <>
      <div className="flex flex-row items-center mt-8 intro-y justify-between">
        <h2 className="mr-auto text-lg font-medium">Kudos</h2>
        <div className="flex">
            ...
            <div className="items-center mt-2 sm:mr-4 xl:mt-0">
              <FormInput
                ...
                onChange={(e) => {
                  setPage(1);
                  if (!e.target.value) { 
                    queryClient.invalidateQueries({ queryKey: KudosApiHooks.getKeyByAlias("getKudosList") });
                  }
                  setFilter({
                    ...filter,
