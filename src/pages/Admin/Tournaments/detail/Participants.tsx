...
export const TournamentDetailParticipants: React.FC<TournamentDetailParticipantsProps> = ({
  tournamentUuid,
  data
}) => {
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      {requestedTeams.length > 0 && <div className="col-span-12 box p-4">
        <div className="flex flex-row items-center justify-between mb-3">
          <h2 className="font-medium">Requested Teams</h2>
            ...
            {requestedTeams.map((team) => (
              <div key={team.uuid} className="bg-white border border-gray-200 rounded-lg py-2 px-4 sm:min-w-[360px] min-w-[60vw] ">
                <div className="flex flex-row items-center justify-between mb-2">
                  <h3 className="font-medium text-gray-900 truncate">Team {team.uuid?.substring(0, 2).toUpperCase()}{team.uuid?.substring(team.uuid.length - 4, team.uuid.length).toUpperCase()}</h3>
      ...
      <div className="col-span-12 box p-4">
        <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2 sm:gap-0">
          <div className="flex flex-row gap-2 items-center">
            <h2 className="font-medium">Registered Teams</h2>
            <Button
              variant="outline-primary"
              size="sm"
              className="text-xs"
                ...
                render: (_, record: any) => (
                  <div className="flex flex-col justify-center gap-1">
                    ...
                    {record.status == "APPROVED" && (
                      <span className="text-xs text-gray-700 bg-gray-100 inline-flex px-2 py-1 w-fit font-medium rounded-full">Payment</span>
                    )}
                  </div>
                )
              },
              {
                ...
                render: (_, record: any) => (
                  <div className="flex justify-center gap-1">
                    {record.status === 'APPROVED' && (
                      <Menu>
                        <Menu.Button className="flex items-center justify-center rounded-md border border-emerald-800 p-1.5 shadow-none hover:bg-emerald-50" disabled={groupIsLocked}>
